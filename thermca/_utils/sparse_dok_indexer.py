"""Vector indexer for sparse.DOK array
"""
...
class DOKVectorIndexer:
    ...
    def slice_getitem(self, keys):
        ...
        for i, key in enumerate(keys):
            if isinstance(key, slice):
                ...
                res_shape[i] = lr if lr > 0 else 0  # if 0: dimension gets preserved but set to zero
            else:  # should be integral or 0d array of integral
                idxs.append((int(key),))
                res_idxs.append((0,))
                res_shape[i] = -1  # dimension gets removed in result
        if any(rs == 0 for rs in res_shape):  # dimension of length zero results in empty array
            new_res_shape = tuple(rs for rs in res_shape if rs >= 0)
            return DOK(shape=new_res_shape, dtype=self._dok.dtype, fill_value=self._dok.fill_value)
        data = self._dok.data
        res_data = dict()
        ...
        if new_res_shape == ():
            ...
            new_dok = DOK(shape=new_res_shape, dtype=self._dok.dtype, fill_value=self._dok.fill_value)
            new_dok.data = res_data
            return new_dok
    ...
    def __getitem__(self, keys):
        ...
        if any(rs == 0 for rs in res_shape):  # dimension of length zero results in empty array
            ...
            return DOK(shape=new_res_shape, dtype=self._dok.dtype, fill_value=self._dok.fill_value)
        non_vec_idxs = [i for i, s in enumerate(res_shape) if s is not None and res_shape[i] > 0]
    ...
    def __setitem__(self, idxs, values):
        ...
        if len(idxs) != self._dok.ndim:
            raise NotImplementedError(f'Index sequences for all {self._dok.ndim} array dimensions needed!')
        ...
        for idx, value in zip(zip(*idxs), values):
            # self._dok._setitem(k, value)  # costly equality checks
...
class DOKArrayIndexer:
    ...
    def __getitem__(self, idxs):
        ...
        data = self._dok.data
        return np.array(
            [data[idx] if idx in data_keys else fill_value
             for idx in zip(*idxs)],
            self._dok.dtype)
